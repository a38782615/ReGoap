    ...
    public class GatherResourceAction : ReGoapAction<string, object>
    {
        ...
        public override List<ReGoapState<string, object>> GetSettings(GoapActionStackData<string, object> stackData)
        {
            ...
            foreach (var pair in stackData.goalState.GetValues())
            {
                ...
                {
                    var newNeededResourceName = pair.Key.Substring(11);
                    if (newNeededResourceName != null && stackData.currentState.HasKey("resource" + newNeededResourceName))
                    {
                        ...
                        {
                            if (wantedResource.resource.GetCapacity() < ResourcePerAction)
                            {
                                continue;
                            }
                            ...
                            {
                                var score = stackData.currentState.TryGetValue("isAtPosition", out object isAtPosition)
                        ...
                        if (!ExpandOnAllResources && best.resource != null)
                        {
