        ...
        public List<INode<ReGoapState<T, W>>> Expand()
        {
            ...
            for (var index = actions.Count - 1; index >= 0; index--)
            {
                ...
                foreach (var settings in settingsList)
                {
                    ...
                    // ReGoapLogger.Debug($"ReGoapNode Expand 1 {possibleAction.GetName()}:{possibleAction.ToString(stackData)}");
                    //获取前提条件
                    var precond = possibleAction.GetPreconditions(stackData);
                    //获取结果
                    var effects = possibleAction.GetEffects(stackData);
                    // any effect is the current Goal
                    // no precondition is conflicting with the Goal or has conflict but the effects fulfils the Goal
                    // no effect is conflicting with the Goal
                    //结果 目标 前提条件 
                    //结果包含目标
                    //目标 和 结果 前提 冲突
                    //目标 和 结果 冲突
                    //检查是否满足条件
                    //ReGoapLogger.Debug($"ReGoapNode Expand 2 {possibleAction.GetName()}\n[GetEffects]:{possibleAction}");
                    var ret = effects.HasAny(Goal);
                    if (ret)
                    {
                        //ReGoapLogger.Debug($"ReGoapNode Expand 3 {possibleAction.GetName()}\n[effects.HasAny](Goal):{Goal} {ret}");
                    }
                    ret = ret && !Goal.HasAnyConflict(effects, precond) ;
                    if (ret)
                    {
                        //ReGoapLogger.Debug($"ReGoapNode Expand 4 {possibleAction.GetName()}\n[Goal.HasAnyConflict](effects, precond):{ret}");
                    }
                    ret = ret && !Goal.HasAnyConflict(effects);
                    if (ret)
                    {
                        //ReGoapLogger.Debug($"ReGoapNode Expand 5 {possibleAction.GetName()}\n[Goal.HasAnyConflict](effects):{ret}");
                    }
                    ret = ret && possibleAction.CheckProceduralCondition(stackData);
                    if (ret)
                    {
                        //ReGoapLogger.Debug($"ReGoapNode Expand 6 {possibleAction.GetName()}\n[.CheckProceduralCondition](stackData):{ret}");
                    }
                    if (ret)
                    {
                        var newGoal = Goal;
                        var node = Instantiate(planner, newGoal, this, possibleAction, settings);
                        expandList.Add(node);
                    }
                }
            }
